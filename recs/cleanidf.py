...
and filters the term list, removing words that aren't in the nltk dictionary and with less the 2 characters length.
...
def main(argv=None):
    ...
    with codecs.open(outfile, 'wb', encoding='utf-8') as wh:
        ...
        wh.write(u'\n'.join([u'{0}:{1}'.format(term,count) \
                    for term, count in term_counts \
                        if wordnet.synsets(term) and len(term) > 2]))
        wh.write('\n')
